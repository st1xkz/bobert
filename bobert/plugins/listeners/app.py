...
class AppButton(miru.View):
    ...
    async def approve_button(self, ctx: miru.ViewContext, button: miru.Button) -> None:
        try:
            ...
            if (
                ctx.interaction.message.embeds[0].footer
                and ctx.interaction.message.embeds[0].footer.text
            ):
                footer_text = ctx.interaction.message.embeds[0].footer.text
            ...
            author = ctx.interaction.message.embeds[0].author
            if author and author.name:
                author_name = author.name
                role = author_name.split(" Application -")[0]
            else:
                author_name = None
                role = None
...
class AppModal(miru.Modal):
    ...
    async def callback(self, ctx: miru.ModalContext) -> None:
        ...
        await app.bot.rest.create_message(
            1085754764693885008, embed=embed, components=view  # Main server channel ID
        )
